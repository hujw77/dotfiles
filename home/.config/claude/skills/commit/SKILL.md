---
name: commit
description: Commits changes with a well-formatted message. Use when user asks to commit, save changes, or create a git commit. Creates a new branch with echo/ prefix if on main/master, then commits with a structured title and Problem/Solution format.
---
...
1. If on `main` or `master`, create a new branch with `echo/` prefix (e.g., `echo/fix-aws-rate-limit`)
